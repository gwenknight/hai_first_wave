...
gen_trans <- function(n_cases, r, so_scen = 1){
  ll <- length(n_cases)
  n_trans <- matrix(0,ll,1) # When did infections occur? 
  n_trans_cases <- matrix(0,ll+25,1) # add in later date could become cases
  ...
  for(i in 1:ll){
   ...
   # print(c("herea",n_cases[i], r))
    ...
    if(n_next_cases > 0 ){
      ...
      if(so_scen == 1){t_symp_hosp <- rlnorm(n_next_cases, meanlog = time_symp_to_hosp_meanlog, sdlog = time_symp_to_hosp_sdlog)}
      if(so_scen == 2){t_symp_hosp <- round(rgamma(n_next_cases, shape = time_symp_to_hosp_shape, scale = time_symp_to_hosp_shape),0)} # time from symp to hosp (i.e. detection)
      if(so_scen == 3){t_symp_hosp <- rlnorm(n_next_cases, meanlog = time_symp_to_hosp_meanlog3, sdlog = time_symp_to_hosp_sdlog3)}
