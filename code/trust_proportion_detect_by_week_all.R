...
### For the four example trusts ( for now )
...
#los <- read.csv("data/first_los.csv")[,-1] # first 4 example places
...
# ggplot(nn, aes(x=cutoff, y = prop_detect, group = interaction(place, week))) + geom_point() + 
#   geom_line(aes(col=week)) + facet_wrap(~place) + 
#   scale_x_continuous("Cutoff threshold") +  
#   scale_y_continuous("Proportion detected", lim = c(0,0.45))
# ggsave("output_all_trusts/prop_detected_by_threshold_byplace.pdf")
# 
# ggplot(nn %>% filter(week < 25), aes(x=week, y = prop_detect, group = interaction(cutoff,place))) + geom_point() + 
#   geom_line(aes(col = factor(cutoff))) + 
#   facet_wrap(~place) + 
#   scale_y_continuous("Proportion detected", lim = c(0,0.5)) + 
#   scale_x_continuous("Week", lim = c(5,25)) + 
#   scale_colour_discrete("Cutoff\nthreshold")
# ggsave("output_all_trusts/prop_detect_over_time.pdf")
# 
# nn %>% filter(place != "ENG") %>% filter(cutoff == 8) %>%
#   summarise(mean = 100*mean(prop_detect),
#             range = 100*range(prop_detect))
# 
# 
# 
# 
...
# 
# ### Plot mean
# nn_mean <- nn %>% group_by(place, cutoff) %>% 
#   filter(cutoff !=5) %>%
#   summarise(mean = mean(prop_detect), sd = sd(prop_detect), median = median(prop_detect))
# 
# ggplot(nn_mean, aes(x=place, y = mean)) + 
#   facet_wrap(~cutoff) + 
#   geom_bar(stat = "identity", aes(fill = place)) + 
#   geom_errorbar(aes(ymin = mean - sd, ymax = mean + sd)) + 
#   geom_hline(yintercept = 0.3, lty = "dashed") + 
#   scale_y_continuous("Proportion detected") + 
#   scale_x_discrete("Place") + 
#   ggtitle("Average proportion detected across weeks") 
# ggsave("output_all_trusts/av_prop_detect_over_weeks.pdf")
