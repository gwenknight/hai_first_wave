...
#### DO FOR ENG / RTH / RR8 / RGT first time around 
...
# If do above in chunks - goes slow
# write.csv(df_noso_eg, "output_all_trusts/noso_numbers_by_trust3.csv") 
# 
# df_noso_eg1 <- read.csv("output_all_trusts/noso_numbers_by_trust1.csv")
# df_noso_eg2 <- read.csv("output_all_trusts/noso_numbers_by_trust2.csv")
# df_noso_eg3 <- read.csv("output_all_trusts/noso_numbers_by_trust3.csv")
# ##### REMEMBER RCD RXC TWICE!
# df_noso_eg1 <- df_noso_eg1 %>% filter(!place == "RCD")
# df_noso_eg2 <- df_noso_eg2 %>% filter(!place == "RXC")
# 
# df_noso_eg <-rbind(df_noso_eg1, df_noso_eg2, df_noso_eg3) 


#df_noso_eg <- read.csv("output_all_trusts/noso_numbers_by_trust.csv")[,-1]
...
g13 <- sum_df_noso %>% group_by(place,cutoff) %>% filter(name == "total_noso") %>%
  summarise(sum_noso = sum(value)) %>% filter(place != "ENG") %>%
...
df <- df_noso_eg %>% filter(place == "ENG") %>% group_by(cutoff) %>% summarise(sumno = sum(n_noso_o),sumn = sum(n_noso), summ = sum(n_missed))
df
write.csv(df, "output_all_groupup/sage_table.csv")
